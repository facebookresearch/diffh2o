...
Datasets = Union[Text2MotionDatasetV2, HumanML3D]
...
def get_model_args(args: FullModelOptions, data: DataLoader):
    ...
    if args.dataset == 'humanml' or args.dataset == 'grab':
        data_rep = 'hml_vec'
        nfeats = 1
    ...
    # if args.drop_redundant:
    #     import trimesh
    #     sbj_vtemp_l = trimesh.load(os.path.join(args.sbj_template_path, gender, sbj_name + '_lhand.ply'),process=False)
    #     sbj_vtemp_r = trimesh.load(os.path.join(args.sbj_template_path, gender, sbj_name + '_rhand.ply'),process=False)
...
def create_gaussian_diffusion(args: FullModelOptions, data: DataLoader):
    steps = 1000
    ...
    return SpacedDiffusion(
        ...
        conf=DiffusionConfig(
            ...
            mano_path=args.mano_model_path,
            batch_size=args.batch_size,
            max_motion_length=data.dataset.t2m_dataset.max_motion_length,
            device=args.device,
        ),
    )
